...
boolean checkController(NESpad controller, CRGB playercolor) {
  ...
  if (state & NES_A) { // when A is pressed, drop the piece
    ...
    while ( leds[currentposition+10] == CRGB(255,255,255) && currentposition+10 < 99 ) { 
      ...
      FastLED.show(50); // show the new state of the game board
      delay(500);
    }
    ...
    return dropped;
      ...
      if (currentposition > 30) { // if there is a place to the left
        leds[currentposition] = CRGB::Black; // set current position to white 
        ...
        leds[currentposition] = CRGB::Black; // set current position to white
        currentposition = 36; // wrap around to right most position
        leds[currentposition] = playercolor; // set new current position to the player's color
      }
      FastLED.show(50); // show the new state of the board
      return dropped;
  
  } else if (state & NES_RIGHT) { // when right is pressed

      if (currentposition < 36) { // if there is a place to the right
        leds[currentposition] = CRGB::Black; // set current position to black 
        currentposition++; // move currentposition to the right
        leds[currentposition] = playercolor; // set new current position to player's color
      } else { // if there is no position to the left
        leds[currentposition] = CRGB::Black; // set current position to black
        currentposition = 30; // wrap around to left most position
        leds[currentposition] = playercolor; // set new current position to the player's color
      }
      FastLED.show(50); // show the new state of the board
      return dropped;
...
void loop() {
  ...
  while (!dropped) // dropped initially set to false, becomes true and exits
  {
    ...
    checkController(controller, playercolor);
    delay(150);
  }
